...
const validDepartmentCodes = ['it@pk', 'it@cr', 'it@hod','it@12'];
    ...
    try {
        ...
        if (admin) {
            ...
            if (hashedPassword === admin.passwd) {
                ...
                // Invalid password
                res.status(401).send('<script>alert("Invalid credentials"); setTimeout(() => { window.location.href = "/Home.html"; }, 1000);</script>');
                // res.status(401).send('Invalid credentials'); // HTTP 401 for unauthorized
            }
        }
        else {
            // Admin not found
            // res.status(404).send('Admin not found'); // HTTP 404 for not found
            res.status(401).send('<script>alert("Admin not found"); setTimeout(() => { window.location.href = "/Home.html"; }, 1000);</script>');
...
app.post('/deleteProjecttec2', async (req, res) => {
    ...
    try {
        ...
        if (!project) {
            res.status(404).send('AI project not found.');
        } else {
            res.send('<script>alert("File deleted successfully"); setTimeout(() => { window.location.href = "/AdminhomePage.html"; }, 1000);</script>');
        }
    } catch (error) {
        console.error('Error deleting AI project:', error.message);
        res.send('<script>alert("Error deleting AI project."); setTimeout(() => { window.location.href = "/AdminhomePage.html"; }, 1000);</script>');
    }
});
...
app.post('/deleteProjecttec3', async (req, res) => {
    ...
    try {
        ...
        res.send('<script>alert("Error deleting ML project."); setTimeout(() => { window.location.href = "/AdminhomePage.html"; }, 1000);</script>');
    }
});
app.get('/viewProjecttec.html', async (req, res) => {
    try {
        ...
        console.error('Error retrieving FullStack projects:', error);
        res.send('<script>alert("Error viewProject FullStack project."); setTimeout(() => { window.location.href = "/Home.html"; }, 1000);</script>');
    }
});
...
app.get('/viewProjecttec3.html', async (req, res) => {
    try {
        ...
        res.send('<script>alert("Error viewProject ML project."); setTimeout(() => { window.location.href = "/Home.html"; }, 1000);</script>');
    }
});
